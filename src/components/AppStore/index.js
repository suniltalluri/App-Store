...
class AppStore extends Component {
  ...
  updateTabItem = tabId => {
    this.setState({ativeTabId: tabId})
    // console.log('TabItem is clicked')
  }

  getFilterAppId = () => {
    const {ativeTabId} = this.state
    const FilterAppId = appsList.filter(
      eachAppItem => eachAppItem.category === ativeTabId,
    )
    return FilterAppId
  }
  ...
  render() {
    const FilterAppId = this.getFilterAppId()
    const {ativeTabId} = this.state

    return (
      <div className="bg_container">
        <div className="APP_Store_card">
          <h1 className="Page_heading">App Store</h1>
          ...
          <ul className="unOrderTabItemList">
            {tabsList.map(eachTabItem => (
              <TabItems
                key={eachTabItem.tabId}
                tabsList={eachTabItem}
                updateTabItem={this.updateTabItem}
                isTabItemClicked={ativeTabId === eachTabItem.tabId}
              />
            ))}
          </ul>
          {/* <hr /> */}
          <ul className="unOrderAPPList">
            {FilterAppId.map(eachApp => (
              <AppItem key={eachApp.id} appsList={eachApp} />
            ))}
          </ul>
        </div>
      </div>
